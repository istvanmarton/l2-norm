...
//	if(index == 0) printf("index: %llu, iMin: %llu, iMax: %llu\n", index, iMin, iMax);
		number = iMin;
		 ...
		 product = 0;
			for(i = 0 ; (iRows - 1) > i; i++){
				logical = (number/d_iNumPower[i]) % 6; //printf("%d, ", logical);
				vect[i] = helper[logical];//logical;
//printf("%d", vect[i]);
					switch(vect[i])
					{
					case 0:
...
cudaGetDeviceProperties(&devProp, 0);
	...
	cudaMalloc((void**)&d_mtx_to_vec, iRows * iCols * sizeof(int));
     	cudaMalloc((void**)&d_mtx_to_vec, iRows * iCols * sizeof(int));
     	unsigned long long int steps, steps_remainder, Inner_num = 1, copyNum, *iNumPower, *d_iNumPower;
	for(i = 0; i < (iRows - 1); i++) {Inner_num *=3;} //printf("Inner num: %llu\n", Inner_num);
	...
	iNumPower[0] = 1;
     	for(i = 1; i < maxRows; i++){iNumPower[i] = iNumPower[i-1] * 3; } //printf("%llu\n", iNumPower[maxRows-1]);
